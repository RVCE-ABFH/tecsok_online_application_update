...
function isCharKey(Control, evt) {
    ...
    //alert(charCode);
    if (charCode == 13 || charCode == 8)//Added to allow backspace in Firefox
        return true;
    if (value != "") {
        if (charCode == 46) {
            if (value.indexOf(".") == -1) {
...
function isNumberKeyValidChar(evt, strValidChars) {
    var strChar = "";
    var charCode = (evt.which) ? evt.which : event.keyCode
...
function isNumberKeyAcceptHypen(Control, evt) {
    var charCode = (evt.which != undefined) ? evt.which : event.keyCode;
    var value = Control.value;
    //Shamli to Accept '-'
    if (value != "" && value.indexOf("-") != 0 && value.lastIndexOf("-") + 1 != value.length && charCode == 45)
        return true;
...
function alphanumeric_only(allowedchar, event) {
    //.charCodeAt();
    var keycode;
    keycode = event.keyCode ? event.keyCode : event.which;
    if ((keycode == 8) || (keycode == 9) || (keycode == 32) || (keycode >= 47 && keycode <= 57) || (keycode >= 65 && keycode <= 90) || (keycode >= 97 && keycode <= 122)) {
        return true;
    }
...
function alphanumericText_only(allowedchar, event) {
    //.charCodeAt();
    var keycode;
...
function isNumberKeywithoutdot(Control, evt) {
    var charCode = (evt.which) ? evt.which : event.keyCode;
    var value = Control.value;
    if (charCode > 31 && (charCode < 48 || charCode > 57))
        return false;
...
function isAlphanumericWithSpace(obj, event) {
    //.charCodeAt();
    var keycode;
    keycode = event.keyCode ? event.keyCode : event.which;
...
function isCharKey1(Control, evt) {
    var charCode = (evt.which) ? evt.which : event.keyCode;
    var value = Control.value;
    // var iChars = "!@#$%^&*()+=-[]\\\;,./{}|\":<>?";
...
function alphanumericwithlength(allowedchar, event, obj, length) {
    var result = true;

    result = alphanumeric_only(allowedchar, event);

    if (result) {
        result = Checkmaxlength(obj, length);
    }
...
function IsCharwithlength(event, obj, length) {
    var result = true;

    result = isCharKey1(obj, event);
...
function isAlphanumeric(obj, event) {
    //.charCodeAt();
    var keycode;
    keycode = event.keyCode ? event.keyCode : event.which;
...
function IsCharwithlengthCount(event, obj, length) {
    //obj + Count for Count display lable
    //    var lbl = document.getElementById("lbl" + obj.id + "Count");
    var result = true;
    var keycode;
    keycode = event.keyCode ? event.keyCode : event.which;
    result = isCharKey1(obj, event);
    if (result || keycode == 8 || keycode == 37 || keycode == 38 || keycode == 39 || keycode == 40) {
        result = Checkmaxlength(obj, length);
        if (obj.value.length > length) {
            obj.value = obj.value.substring(0, length);
        }
    }
    //(document.getElementById(obj.id + "Count")).innerText = "Characters remaining : " + (length - obj.value.length).toString();
    (document.getElementById(obj.id + "Count")).innerHTML = "Characters remaining : " + (length - obj.value.length).toString();
    return result;
}
...
function DateFormatMPSC(keyCode, txt1) {
    // if (n == 1) {
    var isShift = false;
    var seperator = "/";
    var txt = txt1;
...
function isNumberKeyOnly(Control, evt) {
    var charCode = (evt.which != undefined) ? evt.which : event.keyCode;
...
function isDateNumberKey(Control, evt) {
...
function isDate(dateStr) {
    ...
    if (matchArray == null) {
        DisplayAlertMessageBox("please enter date in dd/mm/yyyy format.");
        return false;
    }
